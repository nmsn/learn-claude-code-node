...
// ---- Subagent: 独立上下文，仅返回摘要 ----

async function runSubagent(prompt: string): Promise<string> {
  const subMessages: MessageParam[] = [{ role: 'user', content: prompt }]; // 全新上下文
    ...
    for (const block of content) {
      if (block.type === 'tool_use') {
        ...
        if (block.name === 'task') {
          ...
          process.stdout.write(`\x1b[35m> task (${desc}): ${prompt.slice(0, 80)}\x1b[0m\n`);
